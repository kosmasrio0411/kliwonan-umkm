...
export default function ProductDetailPage() {
  ...
  let thumbnails: Thumbnail[] = [
    { src: product.thumbnail_url || product.imageUrl || 'https://via.placeholder.com/600', alt: product.name }
  ];
  if (product.media && product.media.length > 0) {
    const mediaThumbs = product.media.map((m: any, idx: number) => ({
      src: m.media_url,
      alt: `Media ${idx + 1}`,
      isVideo: m.media_type === 'video'
    }));
    thumbnails = [...thumbnails, ...mediaThumbs];
  }
